...
/**
 * Marketing checkout — uses POLAR_MARKETING_* env vars only.
 * Isolated from the main boilerplate's Polar config (Admin, pricing page, subscriptions).
 */
export async function createMarketingCheckout(baseUrl?: string): Promise<{ url?: string; error?: string }> {
  ...
  if (!productId) {
    return { error: "Marketing product not configured (POLAR_MARKETING_PRODUCT_ID)" };
  }
  if (!token) {
    return { error: "Marketing Polar not configured (POLAR_MARKETING_ACCESS_TOKEN)" };
  }
  ...
  try {
    ...
    if (msg.includes("Product does not exist")) {
      return {
        error:
          "Product not found. Check POLAR_MARKETING_PRODUCT_ID and POLAR_MARKETING_SANDBOX match your Polar dashboard.",
      };
    }
    return { error: msg };
  }
